...
def register(request):
    if request.method == 'POST':    
        ...
        if form.is_valid():
            ...
            messages = render_to_string('accounts/account_verfication_email.html',{
                'user':user,
                'domain': current_site,
            ...
            to_email = email
            send_email =EmailMessage(mail_subject, messages, from_email=settings.EMAIL_HOST_USER, to=[to_email])
            send_email.send()
...
def resetpassword_validate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        messages.success(request,'Plaease reset your password')
...
def order_detail(request,order_id):
    ...
    return render(request, 'accounts/order_deatial.html',context)